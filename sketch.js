const myMainCanvasSketch = (p) => {
  ...
  const PANEL_W = 215, PANEL_H = 340, HEADER_H = 36;
  ...
  // ============================================================
  // SETUP
  // Sound is loaded here for p5.js 2.0.
  // No preload().
  // ============================================================
  p.setup = async () => {
    ...
    colAccept       = p.color(83, 166, 67);
    colReject       = p.color(55, 0, 192);
    colPurple       = p.color(110, 65, 220);
    colPurpleDark   = p.color(55, 0, 192);
    colGold         = p.color(240, 195, 60);

    // ------------------------------------------------------------
    // LOAD SOUNDS - p5.js 2.0 compatible
    // ------------------------------------------------------------
    try {
  ...
  p.mouseWheel = (event) => {
    ...
    if (
      ...
      const maxScroll = Math.max(
        0,
        ALL_ACHIEVEMENTS.length * (ITEM_H + ITEM_GAP) - CONTAINER_H
      );

      achScrollY = p.constrain(
        achScrollY + event.delta * 0.5,
        0,
        maxScroll
      );
  ...
  function applyEffects(effects) {
    for (const statKey of STAT_KEYS) {
      ...
      lastDelta[statKey] =
        delta !== 0 ? delta : lastDelta[statKey];

      if (delta !== 0) {
        stats[statKey] = p.constrain(
          stats[statKey] + delta,
          0,
          100
        );
  ...
  function checkGameOver() {
    for (const statKey of STAT_KEYS) {
      if (stats[statKey] <= 0) {
        gameOver = true;
        endReason =
          `Your ${STAT_LABEL[statKey]} hit rock bottom.`;
        return;
      }
      ...
      if (stats[statKey] >= 100) {
        gameOver = true;
        endReason =
          `Your ${STAT_LABEL[statKey]} maxed out -- and it consumed you.`;
        return;
      }
    }
  }
  ...
  function queueAchievement(id, label) {
    achievementCounts[id] =
      (achievementCounts[id] || 0) + 1;
  ...
  function resolveCard(direction) {
    ...
    if (window.triggerBackgroundWordGrowth) {
    ...
    if (currentSwipeCount >= 5) {
      if (
        typeof window.triggerCorrupt === "function"
      ) {
        window.triggerCorrupt();
      }
    }
  }
  ...
  function drawParticles() {
    ...
    for (const pt of particles) {
      const a =
        p.constrain(pt.life / 40, 0, 1) * 255;
  ...
  function drawValueContainer(
    ...
    let paper = ctx.createLinearGradient(
      0,
      by,
      0,
      by + bh
  ...
  function drawAchievementPopup() {
    ...
    p.text(
      'Day ' +
      achievementPopup.day +
      '  --  earned ' +
      achievementPopup.count +
      ' time' +
      (
        achievementPopup.count > 1
          ? 's'
          : ''
      ),
      x + 40,
  ...
  function drawAchievementBoard() {
    ...
    ALL_ACHIEVEMENTS.forEach(
      (ach, i) => {
        ...
        if (
          iy + ITEM_H < CONTAINER_Y ||
          iy > CONTAINER_Y + CONTAINER_H
        ) {
          return;
        }

        const count =
          achievementCounts[ach.id] || 0;
